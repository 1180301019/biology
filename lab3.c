...
graph g;    //图
HashTable h;
char reads1[500][150];   //文件一的read
char reads2[500][150];   //文件二的read
...
void get_reads()
{
    ...
    {        
        ...
        else
        {
            ...
            // memcpy(reads2[index],buf2,sizeof(char)*150);     //将缓冲区的read写入数组
            index++;    //索引加一
        }
    }
...
void construct_graph()
{
    ...
    for (i = 0;i < 400;i++)  //对于每一个read
    {
        for (j = 0;j < 150-50+1;j++)     //对于每一个k-mer   这里是50mer
        {
            ...
            // memcpy(this_kmer,&(reads1[i][j]),sizeof(char)*51);     
            // memcpy(left_k_1_mer,&(this_kmer[0]),sizeof(char)*50);    
            // memcpy(right_k_1_mer,&(this_kmer[1]),sizeof(char)*50);   
            ...
            if (left_index == -1)     //对于左边的k-1 mer  如果图中还没有这个顶点 就加入这个顶点
            {
                ...
                memcpy(g->vexlist[index_of_new_vert].vertex,left_k_1_mer,sizeof(char)*50);//把左面k-1mer加入图  
                g->vexlist[index_of_new_vert].n = -1;
                g->vexlist[index_of_new_vert].index = index_of_new_vert;    //标记在图中的下标 便于找unipath
                ...
                if (right_index == -1)  //如果右面k-1 mer图中也没有的话 加入图 并加边
                {
                    ...
                    g->n++; //图的顶点下标加一  
                    index_of_new_vert = g->n;      
                    memcpy(g->vexlist[index_of_new_vert].vertex,right_k_1_mer,sizeof(char)*50);//把右面k-1mer加入图
                    g->vexlist[index_of_new_vert].n = -1;
                    g->vexlist[index_of_new_vert].index = index_of_new_vert;    //标记在图中的下标 便于找unipath
